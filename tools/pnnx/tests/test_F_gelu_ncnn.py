...
def test_inference():
    torch.manual_seed(0)
    in0 = torch.rand(1, 16, dtype=torch.float)
    in1 = torch.rand(12, 2, 16, dtype=torch.float)
    in2 = torch.rand(1, 3, 12, 16, dtype=torch.float)
    in3 = torch.rand(1, 5, 7, 9, 11, dtype=torch.float)
    out = []
    ...
    with ncnn.Net() as net:
         ...
         with net.create_extractor() as ex:
            ...
            ex.input("in3", ncnn.Mat(in3.numpy()).clone())
